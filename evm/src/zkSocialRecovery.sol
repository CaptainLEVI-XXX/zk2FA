...
import {HashCheckVerifier} from "./Verifiers/HashCheckVerifier.sol";
import {OtpValidator} from "./OtpValidator.sol";
...
contract ZkSocialRecoveryWallet is IERC721Receiver, OtpValidator {
    ...
    event NewRecoveryProcedure(
        address indexed newProposedOwner, address indexed trusteeInitializer, uint256 currRecoveryRound
    );
    ...
    modifier verifyProofForTrustee(
        ...
        require(IHashCheckVerifier(hashCheckVerifier).verifyProof(a, b, c, Input), "Password proof invalid!");
    ...
    ) OtpValidator(_root, _otpMerkleTreeVerifier) {
    ...
    function setTrusteesPasswords(uint256[] memory _passwordHashes) external isOwner {
        require(Trustees.length == _passwordHashes.length, "Trustees and hashes length diff");
    ...
    function startRecovery(
        ...
        uint256[1] memory Input,
        address newOwner
    ) external isTrustee RecoveryShouldNotBeInProcess verifyProofForTrustee(a, b, c, Input) returns (uint256) {
        ...
        RecoveryProcedure storage recovery = recoveryRoundNumberToProcedure[currentRecoveryNumber];
    ...
    ) external isTrustee RecoveryShouldBeInProcess verifyProofForTrustee(a, b, c, Input) returns (bool success) {
        require(Input[0] == trusteeToPasswordHash[msg.sender], "Wrong password");
        // console.log("Here1");
        require(recoveryRoundNumber <= currentRecoveryNumber, "Wrong Recovery round number");
        // console.log("Here");
        RecoveryProcedure storage recovery = recoveryRoundNumberToProcedure[recoveryRoundNumber];
    ...
    function executeRecoveryChange(
        ...
        uint256[1] memory Input,
        uint256 recoveryRoundNumber
    ) external isTrustee RecoveryShouldBeInProcess verifyProofForTrustee(a, b, c, Input) {
        require(Input[0] == trusteeToPasswordHash[msg.sender], "Wrong password");
        RecoveryProcedure storage recovery = recoveryRoundNumberToProcedure[recoveryRoundNumber];

        require(recovery.numberOfVotesInSupport >= thresholdForRecovery, "Votes Not enough");
    ...
    function cancelRecovery(
        ...
        uint256[1] memory Input,
        uint256 recoveryRoundNumber
    ) external isOwner RecoveryShouldBeInProcess verifyProofForTrustee(a, b, c, Input) {
